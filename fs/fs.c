...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc) {
    ...
    if (filebno < NDIRECT) {
        // can be empty(=0), allocation is up to the caller
        *ppdiskbno = &(f->f_direct[filebno]);
        return 0;
    }

    // if no indirect block allocated
    if (f->f_indirect == 0) {
        if (!alloc)
            return -E_NOT_FOUND;

        int free_blockno;
        if ((free_blockno = alloc_block()) < 0)
            return -E_NO_DISK;

        memset(diskaddr(free_blockno), 0, BLKSIZE);
        f->f_indirect = free_blockno;
...
int file_get_block(struct File *f, uint32_t filebno, char **blk) {
    // LAB 5: Your code here.
    int       r;
    uint32_t *blkno_store;

    if ((r = file_block_walk(f, filebno, &blkno_store, 1) < 0))
        return r;
