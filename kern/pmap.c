...
boot_alloc(uint32_t n) {
    static char *nextfree;  // virtual address of next byte of free memory, *it is a stactic varible*
    char *       result;
...
void page_init(void) {
    ...
    // MEM: 0, free memory..., IO hole, kernel, next free page...
    // **notice**:
...
page_alloc(int alloc_flags) {
    ...
    struct PageInfo *new_list_head = page_free_list->pp_link;
    page->pp_link                  = NULL;
    page_free_list                 = new_list_head;
...
void page_free(struct PageInfo *pp) {
    ...
    if (pp->pp_link != NULL)
        panic("Memory freed twice");
    if (pp->pp_ref != 0)
        panic("Trying to free memory with %d ref remaining", pp->pp_ref);
